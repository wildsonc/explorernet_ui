...
function Docs() {
  ...
  useEffect(() => {
    api.get("api/leveltwo/city").then((res) => setCities(res.data));
  }, []);
  ...
  const SearchCto = useMemo(() => {
    if (debouncedCto && data) {
      ...
      let ctos = data.features.filter((e) =>
        e.properties.name.toLowerCase().includes(debouncedCto.toLowerCase())
      );
      ctos = ctos.slice(0, 5);
      const result: any = [];
      ctos.map((e: any) => {
    ...
    >
      ...
      >
        <form onSubmit={form.onSubmit(handleSubmit)}>
          ...
          <Select
            label="Cidade"
            data={cities ? cities : [{ value: "", label: "Carregando" }]}
          ...
          <Group position="center" mt={10}>
            <Button onClick={() => form.reset()} variant="outline" color="gray">
              Limpar
            </Button>
