...
function Query() {
  ...
  const save = async () => {
    await api.post("api/whatsapp/query", form.values);
  ...
  const test = async () => {
    ...
    const res = await api.post("api/whatsapp/message", {
      ...
      phone2: form.values.test_phone,
      phone3: form.values.test_phone,
    });
  ...
  return (
    <>
      ...
      <Grid>
        <Grid.Col lg={8} md={12}>
          <ScrollArea sx={{ height: 500, position: "relative" }} type="always">
            <Editor
              value={query}
            ...
            />
            <Paper sx={{ position: "absolute", top: 10, right: 10 }}>
              <TemplateValidator
        ...
        <Grid.Col lg={4} md={12}>
          ...
          />
          {template && (
            <TemplateCard templates={template} selected={templateSelected} />
          )}
        </Grid.Col>
      </Grid>
      {queryResult && (
